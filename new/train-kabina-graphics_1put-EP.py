...
def magnetic_calc(x_m, z_m, f_m):
    ...
    hz = sum([h1zkp, h2zkp, h1znt, h2znt, h1zup, h2zup, h1zep, h1zep])
...
def ekran(en):
    ...
    if (abs(y) <= 0.5 * width) and (z >= gr_floor) and (z <= floor + height) and (x > 0) and (x < length):
        # внутри кабины
        if kp_pass or nt_pass or up_pass or ep_pass:
    ...
    def energy():
        ...
        for f in harm.keys():
            ...
            for i in range(0,4):
                ...
                U_h = U * harm[f][1] 
                a = x_m
                ex_ = U_h * a / log(2 * abs(h_p[i]) / d_p[i]) * (1 / ((h_p[i] - z) ** 2 + a ** 2) - 1 / ((h_p[i] + z) ** 2 + a ** 2)) 
                ez_ = U_h / log(2 * abs(h_p[i]) / d_p[i]) * ((h_p[i] - z) / ((h_p[i] - z) ** 2 + a ** 2) + ((h_p[i] + z)) / ((h_p[i] + z) ** 2 + a ** 2)) 
