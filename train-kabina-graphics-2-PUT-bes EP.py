...
def visual_front_locomotive(ext_f):
    ...
    for fr in harm.keys():
        ...
        data = [[
                 el[0][fr][0][0]*el[0][fr][1][0] + el[0][fr][0][1]*el[0][fr][1][1]
...
def glass_reflect(x, y, z):
    ...
    def energy():
        ...
        for f in harm.keys():
            hx1, hz1, ex1, ez1 = 0, 0, 0, 0
            hx2, hz2, ex2, ez2 = 0, 0, 0, 0
            for i in [0,1,2]:
                I_h = I1 * I_p[i] * harm[f][0]
                x_m = y - x_p[i]
                hx1 += I_h / (4 * pi) * (z - h_p[i]) / (x_m ** 2 + (h_p[i] - z) ** 2)
                hz1 += I_h / (4 * pi) * x_m / (x_m ** 2 + (h_p[i] - z) ** 2)
                
                U_h = U1 * harm[f][1] 
                a = x_m
                ex1 += U_h * a / log(2 * abs(h_p[i]) / d_p[i]) * (1 / ((h_p[i] - z) ** 2 + a ** 2) - 1 / ((h_p[i] + z) ** 2 + a ** 2)) 
                ez1 += U_h / log(2 * abs(h_p[i]) / d_p[i]) * ((h_p[i] - z) / ((h_p[i] - z) ** 2 + a ** 2) + ((h_p[i] + z)) / ((h_p[i] + z) ** 2 + a ** 2))   
            
            for i in [3,4,5]:
                I_h = I2 * I_p[i] * harm[f][0]
                x_m = y - x_p[i]
                hx2 += I_h / (4 * pi) * (z - h_p[i]) / (x_m ** 2 + (h_p[i] - z) ** 2)
                hz2 += I_h / (4 * pi) * x_m / (x_m ** 2 + (h_p[i] - z) ** 2)
                
                U_h = U2 * harm[f][1] 
                a = x_m
                ex2 += U_h * a / log(2 * abs(h_p[i]) / d_p[i]) * (1 / ((h_p[i] - z) ** 2 + a ** 2) - 1 / ((h_p[i] + z) ** 2 + a ** 2)) 
                ez2 += U_h / log(2 * abs(h_p[i]) / d_p[i]) * ((h_p[i] - z) / ((h_p[i] - z) ** 2 + a ** 2) + ((h_p[i] + z)) / ((h_p[i] + z) ** 2 + a ** 2)) 
            ...
            h2 += mix(hx2, hz2)
            e2 += mix(ex2, ez2)
        return h1 * e1 * sin(alpha) + h2 * e2 * sin(alpha)
    ...
    kp_sec_dist = Point(y, z).distance(Point(xp_kp2 + xp_mid, h_p[3]))
    ...
    nt_sec_dist = Point(y, z).distance(Point(xp_nt2 + xp_mid, h_p[4]))
    ...
    up_sec_dist = Point(y, z).distance(Point(xp_up2 + xp_mid, h_p[5]))
